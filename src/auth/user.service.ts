...
import { getKey } from "./cryptography/verifyIdentity";
...
export const signIn: RequestHandler = async (req, res) => {
  ...
  if (user && (await validPassword(password, user.password))) {
    ...
    logger.info(loggerInfo("signin", 201, { accessToken, role: user.role }));
    res.status(201).json({ accessToken, role: user.role });
  } else {
    logger.error(
      loggerErr("signin", 401, "Please check your login credentials")
    );
    res.status(401).json({ errMsg: "Please check your login credentials" });
  }
};
...
export const signUp: RequestHandler = async (req, res) => {
  ...
  // 409: 'Conflict';
  if (!!(await userRepo.findOneBy({ email: email }))) {
    logger.error(loggerErr("signup", 409, "User Already Exist. Please Login"));
  ...
  user.password = (await genPassword(password)).hash;
  user.email = email;
  (user.role = UserRole[role] || UserRole.USER), await userRepo.save(user);
  const userfromdb = await userRepo.findOneBy({ email: email });
  const accessToken = userfromdb ? createToken(userfromdb) : "";

  logger.info(loggerInfo("signup", 201, { accessToken, role: user.role }));
  res.status(201).json({ accessToken, role: user.role });
};
...
//   } else {
//     logger.error(loggerErr("updateUser", 401, "cannot update the User info"));
...
//   if (!userfromdb) {
//     logger.error(
//       loggerErr("deleteUserById", 404, `User which ID is "${id}" not found!`)
//     );
//     res.status(404).json({
...
//     } else {
//       logger.error(loggerErr("refreshToken", 404, "Cannot find the user!"));
...
export const checkEmail: RequestHandler = async function (req, res) {
  const user = await userRepo.findOneBy({
    email: (req.body as CheckEmailDto).email,
  });
  if (user) {
